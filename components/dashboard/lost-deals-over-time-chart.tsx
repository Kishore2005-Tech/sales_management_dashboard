...
interface LostDealsOverTimeChartProps {
  deals: Deal[]
}
...
function getLostDealsOverTimeByPeriod(deals: Deal[], period: Period) {
  const lostDeals = deals.filter((d) => d.Status === "Lost")
  const dateMap = new Map<string, { count: number; value: number }>()

  lostDeals.forEach((deal) => {
    const dateField = deal["Deal closed on"] || deal["Deal created"]
    if (dateField) {
      const dateStr = dateField.split(" ")[0]
...
export function LostDealsOverTimeChart({ deals }: LostDealsOverTimeChartProps) {
  const [period, setPeriod] = useState<Period>("month")

  const data = useMemo(() => {
    const allData = getLostDealsOverTimeByPeriod(deals, period)
    ...
    return (
      <Card className="border-border bg-card">
        <CardHeader className="pb-2">
          <CardTitle className="text-base font-medium">Lost Deals Over Time</CardTitle>
        </CardHeader>
        <CardContent className="flex h-[200px] items-center justify-center pt-0">
          <p className="text-sm text-muted-foreground">No lost deals data available</p>
        </CardContent>
      </Card>
    )
  }
