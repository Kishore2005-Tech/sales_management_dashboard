...
interface WonDealsOverTimeChartProps {
  deals: Deal[]
}
...
function getWonDealsOverTimeByPeriod(deals: Deal[], period: Period) {
...
export function WonDealsOverTimeChart({ deals }: WonDealsOverTimeChartProps) {
  const [period, setPeriod] = useState<Period>("month")

  const data = useMemo(() => {
    const allData = getWonDealsOverTimeByPeriod(deals, period)
    ...
    return (
      <Card className="border-border bg-card">
        <CardHeader className="pb-2">
          <CardTitle className="text-base font-medium">Won Deals Over Time</CardTitle>
        </CardHeader>
        <CardContent className="flex h-[200px] items-center justify-center pt-0">
          <p className="text-sm text-muted-foreground">No won deals data available</p>
        </CardContent>
      </Card>
    )
  }
